          ...
          when 'append'
            begin
              ...
              begin
                ...
                File.open("tmp/ariip/#{@template[:payload][:uri]}", 'a+') {|f| f.puts contents }
              rescue Exception => e
                Services::Slog.exception e
              end
              # Append new content to file
              unless @template[:payload][:content].nil? then
                File.open("tmp/ariip/#{@template[:payload][:uri]}", "a+") { |file| file.write(@template[:payload][:content]) }
              end

            # Upload modified file to dropbox
            reply = client.put_file(@template[:payload][:uri], open("tmp/ariip/#{@template[:payload][:uri]}"), true)
