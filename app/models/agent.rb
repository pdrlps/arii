...
class Agent < ActiveRecord::Base
  ...
  def execute
    ...
    begin
      ...
      Services::Slog.info({:message => "Starting checkup for #{identifier}", :module => "Checkup", :task => "integrations", :extra => {:agent => identifier}})
    ...
    begin
      if @checkup[:status] == 100 then
        Services::Slog.info({:message => "Starting integrations processing for #{identifier}", :module => "Checkup", :task => "integrations", :extra => {:agent => identifier, :payload => @checkup[:payload].size}})
        process @checkup
      else
      end
    rescue Exception => e
